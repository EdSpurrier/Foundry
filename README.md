...
- `IParticleAffector` / `ParticleAffectors` — the interface anything that moves particles implements (`Affect(position, velocity, sensitivity, dt)` → new velocity), and the registry affectors join while enabled. `UpdraftTrigger3D` is one; a water current, vortex or explosion could be others without touching `ParticlePhysics`.
